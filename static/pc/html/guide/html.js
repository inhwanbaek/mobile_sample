ui.html = {
	set:{
		init:function () {
			...
			$(document).on("click",".ut-tbl-sort",function(e){ // 리스트 소팅
				if ($(this).is(".dn")) {
					$(this).removeClass("dn").attr({"title":"오름차순정렬"});
				}else{
					$(this).addClass("dn").attr({"title":"내림차순정렬"});
				}
			});
		...
		tit:function(){
			...
			if( document.title == "" || document.title == "UI" ){
				tit = $(".page-hdt .stit").text() || $(".pop-layer .phd .ptit").text() || '' ;
				tit = ""+ tit + " > 교보생명";
				document.title =  tit   ;
		...
		uimenu:{ // uimenu 
			...
			set:function(){
				var lks = 
					...
					'	<a class="lk" href="../../../guide/guide.html" target="_blank">가이드</a>'+
					'	<a class="lk" href="./ui1.html">UI-1</a>'+
					'	<a class="lk" href="./ui2.html">UI-2</a>'+
					'	<a class="lk" href="./pop-sample.html" target="_blank" title="새창열림">Popup</a>'+
					'	<a class="lk" href="./board.html">board</a>'+
					'</div>';
		...
		if ($inchtml.length) {
			$inchtml.each(function(idx){
				...
				$(this).load( inc ,function(response, status, xhr){
					...
					if( incopt && incopt.nav == "false"){
						$(this).find(".location").hide().data("nav",false);
					}
					if( incopt && incopt.part == "corp"){
						$(this).find(".mtog .bt").removeClass("active");
						$(this).find(".mtog .bt.cpy").addClass("active");
					}
					if( incopt && incopt.login == "true"){
						$(this).find(".link .logout").closest("li").show();
						$(this).find(".user .name").show();
						$(this).find(".bt.login").hide();
					}
					if( incopt && incopt.login == "false"){
						$(this).find(".link .logout").closest("li").hide();
						$(this).find(".user .name").hide();
						$(this).find(".bt.login").show();
					}
					...
					if( incopt && incopt.class ){
						...
						$(".header .title .htit").html(incopt.title);

					}
					//lnb fav 관련
					// if($(this).find(".lnb").hasClass("is-fav")){
					// 	$(this).find(".lnb .bt.fav").show();
					// }else{
					// 	$(this).find(".lnb .bt.fav").hide();
					// }
