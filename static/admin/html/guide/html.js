	...
	set:{
		...
		tit:function(){
			...
			if( document.title == "" || document.title == "UI" ){
				// tit = $(".header .title .htit").text() || $(".pop-layer:visible .phd .ptit").text() || '' ;
				tit = "관리자 > 교보생명";
				document.title =  tit   ;
			...
			evt:function(){
				...
				$(document).on("click","nav.uimenu .menu .list>li>a",function(e){
					...
					setTimeout(function(){
						...
						var sc_msid_top = $("[data-sid="+sc_msid+"]").offset().top + $(".container").scrollTop() - $(".header").outerHeight() || 0 ;
						// console.log(sc_msid,sc_msid_top);
	
						$(".container").animate({ scrollTop: sc_msid_top }, 100, function() {
