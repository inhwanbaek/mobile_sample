...
let cntZindex = 1000; // 팝업 z-index 기본값
...
var ui = { //
	...
	gnb: { // GNB tab 방식
		init: function() {
			...
			$(document).on("click", ".header .bt.gnb", function() {
				if( $("body").hasClass("is-pop-gnb")) {
					...
					_this.using("open");
					$("nav.gnb .menu-wrap-acc").css("top", 0);
					$('#gnb .pct').animate({ scrollTop: 0},0);
					$("nav.gnb").removeClass("fixed");
					$("nav.gnb .pct").css("overflow", "auto");
				}
			});
			...
			setTimeout(() => {
				...
				// 2depth 숨기기 테스트
				// $("nav.gnb .cateCont .tit").each(function(){
				// 	let tit_sub = $(this).next("ul");
				// 	// console.log(this);
				// 	// console.log(tit_sub);

				// 	$(this).addClass("open");
				// 	tit_sub.addClass("tit-hide");
				// 	tit_sub.css({"overflow":"hidden", "height":"0", "display":"none"});
				// });

				// $(document).on("click", "nav.gnb .cateCont .tit", function() {
				// 	let tit_sub = $(this).next("ul");
				// 	if ($(this).hasClass("open")) {
				// 		$(this).removeClass("open");
				// 		tit_sub.removeClass("tit-hide");
				// 		tit_sub.css({"overflow":"hidden", "height":"auto", "display":"block"});
				// 	} else {
				// 		$(this).addClass("open");
				// 		tit_sub.addClass("tit-hide");
				// 		tit_sub.css({"overflow":"hidden", "height":"0", "display":"none"});
		...
		set: function() {
			...
			// 메뉴 닫기 / 열기
			$(".bt.gnb").each(function() {
				$(this).find(".blind").remove();
				$(this).attr("aria-label","전체메뉴 열기");
			});
			$("nav.gnb .btn-pop-close").each(function() {
				$(this).find(".blind").remove();
				$(this).attr("aria-label","전체메뉴 닫기");
			});

			// menu-depth1 메뉴 리셋
			$("nav.gnb .menu-wrap-acc .menu-depth1 .bt").each(function() {
				$(this).attr("role","button");
				if($(this).closest("li").hasClass("active")){
					$(this).closest("li>a").attr("aria-expanded",true);
					$(this).attr("title","선택됨");
				} else {
					$(this).closest("li>a").attr("aria-expanded",false);
					$(this).attr("title","");
				}
			});

			// menu-depth1 메뉴 컨트롤
			$(document).on("click", "nav.gnb .menu-wrap-acc .menu-depth1 .bt", function() {
				$(this).closest("li").addClass("active");
				$(this).closest("li>a").attr("aria-expanded",true);
				$(this).attr("title","선택됨");
				$(this).closest("li").siblings(".menu-depth1").removeClass("active");
				$(this).closest("li").siblings(".menu-depth1").find(".bt").attr("aria-expanded",false);
				$(this).closest("li").siblings(".menu-depth1").find(".bt").attr("title","");
				$('nav.gnb .menu-wrap-acc .menu-depth2').animate({ scrollTop: 2},0);
			});

			// menu-depth2 tit 역할
			// $("nav.gnb .menu-wrap-acc .menu-depth2 .tit").each(function() {
			...
			// depth 메뉴 리셋 .sub .sub_depth 클래스 부여
			$("nav.gnb .menu-wrap-acc .menu-depth2 ul li a").each(function() {
				let subCk = $(this).closest("li").find("ul").length;
				...
				// sub_depth
				setTimeout(() => {
					$(this).closest("li").find(">ul>li a[aria-expanded].sub").addClass("sub_depth");
				}, 100);
			});


			// sub 메뉴 컨트롤 * 4,5depth 존재시 첫번째 ul에 적용 =>.first()
			$(document).on("click", "nav.gnb .menu-wrap-acc .menu-depth2 ul li a.sub", function() {
				if($(this).hasClass("open")){
					$(this).removeClass("open");
					$(this).closest("li>a").attr("aria-expanded",false);
					$(this).attr("title","닫힘");
				}else{
					if($(this).hasClass("sub_depth")){
						$(this).closest("li").siblings().find("a.sub").attr("aria-expanded",false);
						$(this).closest("li").siblings().find("a.sub").removeClass("open");
						$(this).closest("li").siblings().find("a.sub").attr("title","닫힘");
					}
					$(this).addClass("open");
					$(this).closest("li>a").attr("aria-expanded",true);
					$(this).attr("title","열림");
			...
			// 접근성 fixed 해제 *접근성 모드 focus 이벤트 아님
			// $(document).on("focus", "nav.gnb .info .m8 .bt", function() {
			// 	$('#gnb .pct').animate({ scrollTop: 0},0);
			// 	$("nav.gnb").removeClass("fixed");
			// 	$("nav.gnb .pct").css("overflow", "auto");
			// 	console.log(true);
		...
		cateScroll:function(){
			$("#gnb:not(.abt) .pct").on("scroll",function(){
				let hi_phd = Math.floor($("#gnb .phd .in").innerHeight());
				let hi_info = Math.floor($("#gnb .pct .info").innerHeight());
				// let hi_top = hi_phd + hi_info;
				let scrTop = $(this).scrollTop();
				if (scrTop >= hi_info) {
					$("nav.gnb .menu-wrap-acc").css("top", hi_phd);
					$("nav.gnb .pct").css("overflow", "hidden");
					$("nav.gnb .menu-wrap-acc .menu-depth2").scrollTop(0);
					$("nav.gnb").addClass("fixed");
					$("nav.gnb").addClass("delay-lock");

				}
			});
			$("nav.gnb .menu-wrap-acc .menu-depth2").on("scroll",function(){
				if ( $("nav.gnb").hasClass("delay-lock") ) {
					setTimeout(() => {
						$("nav.gnb").removeClass("delay-lock");
					}, 100);
				} else {
					let hi_phd = Math.floor($("#gnb .phd .in").innerHeight());
					let hi_info = Math.floor($("#gnb .pct .info").innerHeight());
					let hi_top = hi_phd + hi_info;
					let scrTop_d2 = $(this).scrollTop();
					if (scrTop_d2 < 1 && $("nav.gnb").hasClass("fixed") ) {
						$("nav.gnb .menu-wrap-acc").css("top", hi_top);
						$("nav.gnb .pct").css("overflow", "auto");
						setTimeout(() => {
							$("nav.gnb").removeClass("fixed");
							$("nav.gnb .menu-wrap-acc").css("top", 0);
	...
	ly:{ // 레이아웃
		...
		floating:{ // 우하단 고정되있는 버튼들
			...
			evt:function(){
				...
				$(document).on("click",".quicknav .xbox",function(e){
					$(".quicknav").addClass("hide");
				});


				$(".quicknav .qmenu").attr("aria-expanded","false");
			},
			...
			openMenu:function(){  // 퀵메뉴
				if($(".quicknav").hasClass("open")){
					...
					$(".quicknav .qmenu").attr("aria-expanded","false");
					// [접근성] 퀴메뉴 추가 2022-10-27
					$(".header").attr("aria-hidden","false");
					$(".container.page").attr("aria-hidden","false");
				}
				else {
					...
					$(".quicknav .qmenu").attr("aria-expanded","true");
					// [접근성] 퀴메뉴 추가 2022-10-27
					$(".header").attr("aria-hidden","true");
					$(".container.page").attr("aria-hidden","true");
				}
			}
		},
		botnav:{  // 하단 메뉴
	...
	steps:{  // UI-CRPMGNL4191AS.html
		...
		set:function(){
			$(".ut-steps.d").each(function(){
				...
				// 접근성 수정 2022-07-29
				var tit_txt = $this.find(".tit").text();
				...
				if( parseInt(tot) <= 2 ){  //  전체 스텝이 2단계 이하일때 프로그래스 숨김처리;
					...
					$(this).closest(".ut-steps").find(".tit").attr("aria-hidden","true");
					// $(this).closest(".ut-steps").find(".tit").remove();
				}

				$bar.css({"width":rest+"%"});
				$box.attr({"role":"img","aria-label":"총 "+tot+"단계 중 현재 "+num+"단계 " + tit_txt});
			});
		}
	},
	...
	topfix:{ // 스크롤시 상단고정할것
		...
		set:function(){
			...
			// 2022-08-16 상품설명확인서 ipdgn-step step fixed 되는 경우때문에 if문 추가함
			if( !fixedbox.closest('.ipdgn-step') ){
	...
	gnbtopfix:{ // 스크롤시 상단고정할것 (모바일 사용 안함)
		init:function(){
			//this.evt();
			//this.set();
		},
	...
	fsize:{ // 폰트크기 조절 UI
		...
		open:function(els){
			...
			$("[data-ui-fsize='pop'] button[value='"+fsize+"']").attr("title","선택됨");
	...
	form:{  //  폼요소
		...
		select:{
			...
			set:function(){
				$(".select").each(function(){
					...
					if( !$(this).find(".btsel").length ) {
						...
						// $(this).attr("tabindex","0"); // 접근성 focus 주석처리 2022-11-25
					}
			...
			close:function(els){
				...
				$(els).closest(".select").focus(); // 접근성 focus 수정 2022-09-06
				// $(els).closest(".select").find(".btsel").attr("tabindex","0").focus(); // 접근성 focus 수정 2022-09-06
	...
	accd:{ // 아코디언 UI 접근성 수정 2022-08-30
		init: function() {
			this.using();
			this.set();
		},
		set:function(){
			...
			// 아코디언 접근성 수정 2022-08-30 ctrl-accd 추가
			$(".ut-accd").each(function(){
				var clickable = $(this).attr("data-clickable");
				if(clickable == 'false'){
					$(this).find(">li:not(.expt)>.hbox .btn-tog").addClass("ctrl-accd").attr("role", "button");
				} else {
					$(this).find(">li:not(.expt)>.hbox").addClass("ctrl-accd");
					// $(this).find(">li:not(.expt)>.hbox .btn-tog").attr("role", "none").attr("aria-hidden", "true");
				}
			});

			// 아코디언 접근성 수정 2022-08-30 btn-tog 초기화
			...
			// 아코디언 접근성 수정 2022-08-30
			let idx = 0;
			let accd_hd = document.querySelectorAll(".ut-accd>li:not(.expt)");
			if (accd_hd.length >= 1) {
				accd_hd.forEach(function(item) {
					idx++;
					// let hbox = item.querySelector(".hbox");
					let hbox = item.querySelector(".ctrl-accd");
					let cbox = item.querySelector(".cbox");
					let tit = item.querySelector(".hbox [data-accd-title]");
					// hbox
					if (!hbox == "") {
						let btn = hbox.querySelector(".btn-tog");
						let btn_accd = item.querySelector(".btn-tog.ctrl-accd");
						let chk = item.querySelector(".hbox .checkbox .txt");
						hbox.setAttribute("role", "button");
						hbox.setAttribute("id", `accd-hb-${idx}`);
						hbox.setAttribute("aria-controls", `accd-cb-${idx}`);
						// 일반적인 아코디언 헤더
						if (!btn == "") {
						...
						if (item.classList.contains("open")) {
							hbox.setAttribute("aria-expanded", "true");
						} else {
							hbox.setAttribute("aria-expanded", "false");
						}
						// 예외 1. 아코디언 헤더에 툴팁 버튼 포함된 형태
						if (!tit == "" && !btn_accd == "") {
							let text = tit.innerText;
							btn_accd.setAttribute("aria-label", `${text} 상세보기`);
							// console.log(text);
							// console.log(btn_accd);
						}
						// 에외 2. 아코디언 헤더에 체크박스 포함된 형태
						if (!chk == "" && !btn_accd == "") {
							let text = chk.innerText;
							btn_accd.setAttribute("aria-label", `${text} 상세보기`);
							// console.log(text);
							// console.log(btn_accd);
						}
					...
					// cbox
					if (!cbox == "") {
						cbox.setAttribute("role", "region");
						cbox.setAttribute("id", `accd-cb-${idx}`);
						cbox.setAttribute("aria-labelledb", `accd-hb-${idx}`);
						// console.log(cbox);
					}
				});
			}
			...
			$(document).on("click", ".ut-accd .ctrl-accd", function() {
				...
				if( type == "tog" ){
					if( $cbox.is(":hidden") ){
						...
						$cbox.slideDown(100,function(){
							...
							$li.addClass("open").find(".ctrl-accd").first().attr("aria-expanded",true);
							ui.lock.using(false);
							_this.ckaccd(id);
						});
					}else{
						...
						$cbox.slideUp(100,function(){
							...
							$li.removeClass("open").find(".ctrl-accd").first().attr("aria-expanded",false);
							ui.lock.using(false);
							_this.ckaccd(id);
						});
					}
				...
				if( type == "accd" ){
					$(this).closest(".ut-accd").find(">li.open").not("li.expt").find(">.cbox").slideUp(100,function(){
						...
						$(this).closest(".ut-accd").find(">li.open").removeClass("open").find(".ctrl-accd").first().attr("aria-expanded",false);
						_this.ckaccd(id);
					});
					if( $cbox.is(":hidden") ){
						ui.lock.using(true);
	...
	accd_old:{ // 아코디언 UI 접근성 수정전 2022-08-30
		init: function() {
			this.using();
			this.set();
		},
			...
			$(document).on("click","[data-tog-cls='btn']",function(e){
				...
				if( $box.is("."+val) ) {
					$ctn.slideUp(100,function(){
						...
						$btn.removeClass(val).attr("aria-expanded",false)
						$ctn.removeClass(val);
					});
				}else{
					$ctn.slideDown(100,function(){
						...
						$btn.addClass(val).attr("aria-expanded",true);
						$ctn.addClass(val);
		...
		set:function(){
			$("[data-tog-cls='box']").each(function(){
				...
				if($box.hasClass("open")) {
					if($box.find("> [data-tog-cls='btn']").length) {
						$box.find("> [data-tog-cls='btn']").addClass("open");
					}
				}
			...
			$("[data-tog-cls='btn']").each(function(){
				...
				// if( !$btn.find(".btxt").length ){
				// 	$btn.append('<span class="btxt"></span>');
				// }
				if( $btn.find(".btxt").length ){
					$(".btxt").remove();
				}
				...
				if( $btn.is(".open") ){
					...
					$btn.attr("aria-expanded",true).find(".btxt")
				}else{
					// $btn.attr("aria-expanded",false).find(".btxt").text("열기");
					$btn.attr("aria-expanded",false).find(".btxt")
				}
			});
	...
	lock:{ // 스크롤 막기,풀기
		sct:0,
		stat:false,
		els:".pop-layer:visible  , .ui-confrim:visible , .ui-alert:visible , .floatbots.fl-otp .pbt.opt .opt-scrollbox:visible",
	...
	slides:{ // 스와이프 슬라이드
		...
		set:function(){
			...
			//[D] 2022-07-21 웹접근성 모든 swiper에 role= / tabindex  추가
			$('.swiper-container .swiper-wrapper .swiper-slide a').each(function(i){
				$(this).attr("tabindex", "0");
				$(this).parent().attr("role", "banner");
			});
	...
	alert:function(msg,params){ // 커스텀 알럿
		...
		var lyAlert =
		'<article aria-live="assertive" class="ui-alert ' + opt.cls + '">' +
		...
		// [접근성] ios 이중 팝업(alert, confirm) 포커스 2022-10-24
		if (opt.msg) {
			$(".pop-layer.on").attr("aria-hidden", "true");
		}
		...
		function alertClose(){
			$(".pop-layer.on").attr("aria-hidden", "false"); // [접근성] ios 이중 팝업(alert, confirm) 포커스 2022-10-24
			$(".ui-alert").remove();
	...
	confirm:function(msg,params){ // 커스텀 컨펌
		...
		var lyConfirm =
		'<article aria-live="assertive" class="ui-confrim ' + opt.cls + '">' +
		'	<div class="pbd" tabindex="0">'+
		'		<div class="phd"><span class="tit">'+opt.tit+'</span></div>'+
		'		<div class="pct"><div class="msg">'+opt.msg+'</div></div>'+
		'		<div class="pbt">'+
	...
	popup:{ // 레이어팝업
		...
		open: function(id,params) {
			let bottomUpSheetId = ["pop-app-interest","pop-dti1693","pop-dti1694", "gnb", "pop-notice-stfu", "sys-error-notice"];
			if((bottomUpSheetId.indexOf(id) > -1 || id.indexOf("pop-notice-evt") === 0)&& _nowPageCd === "DTMCONLNLM001" && localStorage.getItem("main.bottom.myDeck") !== null && localStorage.getItem("main.bottom.myDeck") !== "") {
				native.getAppInfo({}, function (result) {
					if (result.appInfo.version >= "1.0.3" && _USER.isLogin) {
						native.hideBottomUpSheet(function(){
							native.isBottomUpSheetOk = false;
						});
					}
				});
			}
			...
			$("#" + id).attr("role","dialog").attr("aria-modal","true").addClass(_this.opt.direct).css({ zIndex: cntZindex }).fadeIn(10,function(){
				...
				$(this).siblings(".pop-layer").attr("aria-hidden", "true");
				// console.log(cntZindex);
			});
		...
		close: function(id,params) {
			...
			$("#"+id).siblings(".pop-layer.on").attr("aria-hidden", "false");
			// data-layer 포커스
			// let target = document.querySelectorAll(`[data-layer=${id}]`);
			// if (target.length >= 1) {
			// 	target.forEach(function(item) {
			// 		// let name = item.dataset.layer;
			// 		// console.log(id);
			// 		// console.log(name);
			// 		item.focus();
			// 	});
			...
			let bottomUpSheetId = ["pop-app-interest","pop-dti1693","pop-dti1694", "gnb", "pop-notice-stfu", "sys-error-notice"];
			if(bottomUpSheetId.indexOf(id) > -1 && _nowPageCd === "DTMCONLNLM001" && localStorage.getItem("main.bottom.myDeck") !== null && localStorage.getItem("main.bottom.myDeck") !== "") {
		...
		resize:function(){
 			$(".pop-layer:visible").each(function(){
				...
				if( $pop.find(".pbt:visible").length || $pop.find(".floatbots>.inr:visible").length ) {
					...
					if(pbtCnt>0) pdBottom = pbtnH+floatH+phdH; //.pbt
	...
	inputFocus:{ // 키패드 올라올때 인풋에 포커스 이동
		...
		focus:function(){
			...
			if(len_cit) {
				...
				const _space = 16;
				// step 있을 경우
				const step = document.querySelector(".ut-topfix");
				let _sth;
				if (!step == "") {
					_sth = step.clientHeight;
				}
				for (let i = 0; i < len_cit; i++) {
					let _el = _cit[i];
					_el.addEventListener("click", function () {
						if (!_sth == "") {
							let pos = (window.pageYOffset + _el.getBoundingClientRect().top) - (_hdh + _space + _sth);
			...
			if(len_lit && len_litnf) {
				for (let i = 0; i < len_lit; i++) {
					...
					_el.addEventListener("click", function () {
						...
						if (!step == "") {
							_sth = step.clientHeight;
							pos = (_prt.scrollTop + _el.getBoundingClientRect().top) - (_phdh + _space + _sth);
							console.log(pos);
						} else {
							pos = (_prt.scrollTop + _el.getBoundingClientRect().top) - (_phdh + _space);
							console.log(pos);
						}
	...
	wacc:{ //접근성 추가 코드
		...
		set: function() {
			...
			// 새창열기
			$(".bt.target, .ico-elink, a[target='_blank']").each(function(){
				$(this).attr("title", "새창열림");
			});

			// logo 수정
			// let acc_logo = document.querySelectorAll(".logo");
			// if (acc_logo.length >= 1) {
			// 	acc_logo.forEach(function(item) {
			// 		let blind = item.querySelector(".blind");
			// 		if (!blind == "") {
			// 			blind.remove();
			// 		}
			// 		item.setAttribute("title", "교보생명");
			...
			// role button
			let acc_link = document.querySelectorAll("body a:not([data-ui-tab-btn])");
			if (acc_link.length >= 1) {
				acc_link.forEach(function(item) {
			...
			// 보험상품 선택 버튼 대체 텍스트
			let acc_prd_txt = document.querySelectorAll(".hdt .bts .bt-tog > i.blind");
			if (acc_prd_txt.length >= 1) {
				acc_prd_txt.forEach(function(item) {
					item.innerText = "상품 선택";
				});
			}

			// 임의탐색
			let acc_label = document.querySelectorAll("label.lb");
			if (acc_label.length >= 1) {
				acc_label.forEach(function(item) {
			...
			// 주민등록번호 패턴
			// 접근성 not-hide 추가 2022-09-01
			let acc_backnum = document.querySelectorAll("input.ln:not(.not-hide)");
			if (acc_backnum.length >= 1) {
				acc_backnum.forEach(function(item) {
					let att = item.attributes;
					if (att.readonly && att.pattern) {
			...
			// 필수입력 텍스트 초점
			let acc_essential = document.querySelectorAll(".ast");
			if (acc_essential.length >= 1) {
				acc_essential.forEach(function(item) {
					let hdt3 = item.closest(".hd-tit3");
					if (!hdt3 == "") {
						hdt3.setAttribute("role", "text");
						// console.log(hdt3);
					}
					let hdt4 = item.closest(".hd-tit4");
					if (!hdt4 == "") {
						hdt4.setAttribute("role", "text");
						// console.log(hdt4);
					}
					let hdt5 = item.closest(".hd-tit5");
					if (!hdt5 == "") {
						hdt5.setAttribute("role", "text");
						// console.log(hdt5);
					}
					let catefl = item.closest(".cate > .fl");
					if (!catefl == "") {
						catefl.setAttribute("role", "text");
						// console.log(catefl);
					}
				});
			}
			...
			// pop-layer container 밖으로 강제 이동
			// let acc_layer = document.querySelectorAll(".pop-layer:not(.gnb)");
			// let acc_wrap = document.querySelectorAll(".wrap");
			// if (acc_layer.length >= 1 && acc_wrap.length >=1) {
			// 	acc_layer.forEach(function(item) {
			// 		acc_wrap[0].appendChild(item);
			// 	});
			// }
			...
			// [접근성] 모바일 메인 상태 추가 2022-11-13
			setTimeout(() => {
				// viewbox
				let acc_viewbox = document.querySelectorAll(".main .viewbox");
				// specbox
				let acc_specbox = document.querySelectorAll(".main .specbox .nudge:not(.first)");
				// viewbox_section
				let acc_viewbox_section = document.querySelectorAll(".main .viewbox > div");

				if (acc_viewbox.length >= 1) {
					acc_viewbox.forEach(function(item) {
						let btn = item.querySelector(".btn-tog");
						// acc_main_state
						function acc_main_state(el){
							if (el == "open") {
								// console.log("open");
								// btn
								btn.setAttribute("aria-expanded", "true");
								btn.setAttribute("aria-label", "메인 추가 정보 영역 닫기");
								// console.log(btn);
								// specbox
								if (acc_specbox.length >= 1) {
									acc_specbox.forEach(function(item) {
										item.setAttribute("aria-hidden", "true");
										// console.log(item);
									});
								}
								// viewbox_section
								if (acc_viewbox_section.length >= 1) {
									acc_viewbox_section.forEach(function(item) {
							...
							if (el == "close") {
								...
								btn.setAttribute("aria-label", "메인 추가 정보 영역 열기");
								// console.log(btn);
								// specbox
								if (acc_specbox.length >= 1) {
									acc_specbox.forEach(function(item) {
						...
						btn.addEventListener("click", e => {
							...
							if (item.classList.contains("close")) {
								acc_main_state("close");
							}
						});
					});
				}
			}, 500); // 메인 setTimeout 1초
			// [접근성] 모바일 메인 상태 추가 2022-11-13

			// 수익률 상승, 하락 대체 텍스트 flu
			let acc_flu = document.querySelectorAll(".flu");
			if (acc_flu.length >= 1) {
				acc_flu.forEach(function(item) {
					let rate = item.querySelector(".ico-rate");
					let blind = item.querySelector(".blind");
					item.setAttribute("role", "text");
					if (!blind == "") {
						blind.remove();
					...
					if (!rate == "") {
						if (item.classList.contains("dw")) {
							rate.setAttribute("aria-label", "수익률 하락");
							// console.log(rate);
						} else if (item.classList.contains("up")) {
							rate.setAttribute("aria-label", "수익률 상승");
							// console.log(rate);
						}
					}
				});
			...
			// ut-hash-box active 상태값
			let acc_hash = document.querySelectorAll("main:not(.main) .ut-hash-box .ut-hash");
			if (acc_hash.length >= 1) {
				acc_hash.forEach(function(item) {
					if (item.classList.contains("active")) {
						item.setAttribute("title", "선택됨");
						// console.log(item);
					}
					$(document).on("click", ".ut-hash-box", function() {
						if (item.classList.contains("active")) {
							item.setAttribute("title", "선택됨");
						} else {
			...
			// 관심상품 bt fav
			let acc_fav = document.querySelectorAll(".header .rdt .bt.fav");
			if (acc_fav.length >= 1) {
				acc_fav.forEach(function(item) {
					item.setAttribute("aria-label", "관심상품 보기");
					// console.log(item);
				});
			}

			// bt-tog bt-tog2
			let acc_tog2 = document.querySelectorAll(".bt-tog.bt-tog2");
			if (acc_tog2.length >= 1) {
				acc_tog2.forEach(function(item) {
					item.setAttribute("aria-label", "내용 상세보기");
					console.log(item);
				});
			}
			...
			// ut-range type=button
			let acc_utlabel = document.querySelectorAll(".ut-range label.radio input[type=button]");
			if (acc_utlabel.length >= 1) {
				acc_utlabel.forEach(function(item) {
					let label = item.closest("label");
					let txt = label.querySelectorAll(".txt");
					let target = txt[0];
					// console.log(target);
					if (txt.length >= 1) {
						if (target.classList.contains("checked-button-active")) {
	...
	appEffect: { // 어플용 버튼 애니메이션 테스트
		init: function() {
			const app_chk = document.querySelector("body");

			if(app_chk.classList.contains("app")) {
				setTimeout(() => {
					this.evt();
				}, 500);
			}
		...
		evt: function() {
			// 버튼, 링크 초기화
			let el_btn = document.querySelectorAll(".bt:not(.gnb), .bt-logout, a.my-kyobo, button.my-keyword, .nudge a, .nudge-user a, .dict-detailBanner");
			// console.log(el_btn.length);
			if (el_btn.length >= 1) {
				el_btn.forEach(function(item) {
					let el_taint = document.createElement("span");
					el_taint.classList.add("taint");
					item.classList.add("ui-app-ripple");
					item.appendChild(el_taint);
					// console.log(item);
					// console.log(el_taint);
					let menu_btn = item.closest(".menu-depth1");
					if (!menu_btn == "") {
						item.classList.remove("ui-app-ripple");
			...
			let x;
			let y;
			$(document).on("click",".ui-app-ripple",function(e){
				// e.preventDefault();
				...
				if (taint.length > 0) {
					...
					setTimeout(() => {
						taint.css({ top: "", left: "", height: "", width: "" })
					  taint.removeClass("drop");
					}, 450);
				}
				return false;
			...
			// remove drop
			// let rst_drop = document.querySelector(".bt.gnb");
			// let rst_taint = document.querySelectorAll(".taint");
			// rst_drop.addEventListener("click", e => {
			// 	if (rst_taint.length >= 1) {
			// 		rst_taint.forEach(function(item) {
