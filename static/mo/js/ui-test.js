...
var ui = { //
	...
	gnb: { // GNB tab 방식
		...
		set: function() {
			...
			$(document).on("click", "nav.gnb .menu-wrap-acc .menu-depth1 .bt", function() {
				...
				$(this).closest("li").siblings(".menu-depth1").attr("aria-expanded",false);
			...
			$("nav.gnb .menu-wrap-acc .menu-depth2 ul li a").each(function() {
				...
				// sub_depth
				setTimeout(() => {
					$(this).closest("li").find(">ul>li[aria-expanded]>.sub").addClass("sub_depth");
				}, 100);
			});
	...
	gnb_old: { // 삭제 예정
		init: function() {
			...
			let gnb_old = document.querySelectorAll(".gnb .menu-wrap");
		...
		set: function() {
			...
			$("nav.gnb .cateCont ul li a").each(function(){
				...
				// sub_depth
				setTimeout(() => {
					// var depth_chk = $(this).closest("li").find(">ul>li[aria-expanded] .sub").length;
					// console.log(depth_chk);
					$(this).closest("li").find(">ul>li[aria-expanded]>.sub").addClass("sub_depth");
				}, 100);
			...
			// cateCont 최소 높이 조정
			$("#gnb:not(.abt) .cateCont").each(function(){
				var _contH = $(this).innerHeight();

				if(_contH < _wH) {
					var _nH = _wH + 1;
					$(this).css("min-height",_nH);
					// console.log(_contH);
					// console.log(_wH);
			...
			// 서브메뉴 리셋
			function reset_gnb_sub() {
				$("nav.gnb .cateCont ul li a.sub").closest("li").find(".btxt").text("열기");
				$("nav.gnb .cateCont ul li a.sub").closest("li").attr("aria-expanded",false);
			...
			menu.on("click", "a", function(e){
				...
				var is_info = $("#gnb .pct .info").length;
			...
			$(document).on("click", "nav.gnb .cateCont ul li a.sub", function() {
				...
				if($(this).hasClass("open")){
					...
					if($(this).hasClass("sub_depth")){
						$(this).closest("li").siblings().find(".btxt").text("열기");
						$(this).closest("li").siblings().attr("aria-expanded",false);
						$(this).closest("li").siblings().find("a.sub").removeClass("open");
						$(this).closest("li").siblings().find("ul").slideUp(0);
					}
					else {
						// reset_gnb_sub();
					}
		...
		cateScroll:function(){
			...
			$('#gnb:not(.abt) .cates').on("scroll",function(){
				...
				if(catesTop === 0) {
					$(".menu-wrap").removeClass("fixed");
					// $(".list.other").removeClass("fixed");
					$('#gnb .pct').css({"overflow":"auto"});
					$('#gnb .pct').animate({ scrollTop: baseH},0);
					$('#gnb .pct').animate({ scrollTop: 0},500);
				}
			});
		},
	...
	wacc:{ //접근성 추가 코드
		...
		set: function() {
			...
			// 새창열기
			$(".bt.target, a[target='_blank']").each(function(){
			...
			// role button
			let acc_link = document.querySelectorAll("body a");
			...
			// 주민등록번호 패턴
			let acc_backnum = document.querySelectorAll("input.ln");
			...
			// mo main viewbox btn-tog
			let acc_viewbox = document.querySelectorAll(".viewbox");
